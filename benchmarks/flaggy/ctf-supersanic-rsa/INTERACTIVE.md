...
```bash
harbor run -p benchmarks/flaggy/ctf-supersanic-rsa -a terminus-2 -m <model>
```

To work in the same generated environment yourself:

```bash
flaggy interactive benchmarks/flaggy/ctf-supersanic-rsa --tool shell
flaggy interactive benchmarks/flaggy/ctf-supersanic-rsa --tool pi
```
